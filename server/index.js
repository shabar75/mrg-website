...
app.post('/api/consultation', consultationLimiter, async (req, res) => {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.error(
        '[Resend] Missing RESEND_API_KEY environment variable.'
      );
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
    <tr>
      ...
      <td style="padding: 8px 0;">${escapeHtml(fullName.trim())}</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="padding: 8px 0;">${escapeHtml(phone.trim())}</td>
    </tr>
    <tr>
      <td style="padding: 8px 0; font-weight: bold;">Company:</td>
      <td style="padding: 8px 0;">${escapeHtml(company.trim() || 'N/A')}</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td style="padding: 8px 0;">${escapeHtml(preferredContact)}</td>
    </tr>
    <tr>
  ...
  <div style="background: #f7fafc; padding: 15px; border-radius: 6px; border-left: 4px solid #3182ce; white-space: pre-wrap;">
${escapeHtml(message.trim())}
  </div>

  <p style="margin-top: 30px; font-size: 12px; color: #718096;">
    This email was sent from the MRG Solutions website consultation form.
  </p>
</body>
</html>
`.trim();
    ...
    console.log(
      `[Consultation] Email sent for ${email.trim()} at ${submittedAt}. Resend ID: ${
        data?.id || 'unknown'
      }`
    );
